...
describe('RollingWheel Component', () => {
	...
	test('renders with custom props', () => {
		...
		render(
			<RollingWheel result="ABC" resultType="TEXT" size="large" placeholder={customPlaceholder} />
		);
	...
	test('starts animation automatically when autoStart is true', () => {
		...
		render(<RollingWheel result="123" autoStart={true} onStart={onStartMock} />);
	...
	test('does not start animation when autoStart is false', () => {
		...
		render(<RollingWheel result="123" autoStart={false} onStart={onStartMock} />);
	...
	test('can be started programmatically using ref', () => {
		...
		render(<RollingWheel ref={ref} result="123" autoStart={false} onStart={onStartMock} />);
	...
	test('restarts animation when result changes', () => {
		const onStartMock = vi.fn();
		const { rerender } = render(<RollingWheel result="123" onStart={onStartMock} />);

		// onStart should be called once for initial render
		expect(onStartMock).toHaveBeenCalledTimes(1);

		// Change the result prop
		rerender(<RollingWheel result="456" onStart={onStartMock} />);
	...
	test('uses custom render function when provided', () => {
		...
		render(<RollingWheel result="123" render={customRender} />);
